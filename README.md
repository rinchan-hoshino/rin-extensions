...
- keeps bare `rin usage` as a compact Codex-style text status with 20-segment remaining bars and reset times, while Chat `/usage` returns the quota card with a dated, per-local-day 7-day USD-equivalent usage trend as a PNG and uses the same compact status as its text fallback;
